...
class MetaCorgiSnacks
  def initialize(snack_box, box_id)
    @snack_box = snack_box
    @box_id = box_id
    @snack_box.methods.grep(/_(.*)_info$/) { MetaCorgiSnacks.define_snack($1) }
  end

  # def method_missing(name, *args)
  #   info = @snack_box.send("get_#{name}_info", @box_id, *args)
  #   taste = @snack_box.send("get_#{name}_tastiness", @box_id, *args)
  #   puts info, taste
  # end
